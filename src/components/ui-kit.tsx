...
export function PageHeader({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: ReactNode }) {
  return (
    <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
      <div>
        <div className="text-[10px] uppercase tracking-widest text-muted-foreground">MerchIQ · Mission Control</div>
...
export function Panel({ title, right, children, className, dense }: { title?: string; right?: ReactNode; children: ReactNode; className?: string; dense?: boolean }) {
  return (
    <div className={cn("panel", className)}>
      {title && (
        <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
          <div className="text-[11px] font-medium uppercase tracking-widest text-muted-foreground">{title}</div>
          {right}
        </div>
      )}
...
export function Btn({
  ...
  const base = "inline-flex items-center justify-center gap-1.5 rounded-md font-medium transition-colors disabled:opacity-50 border";
  const variants = {
    ...
    ghost: "border-transparent bg-transparent text-muted-foreground hover:text-foreground hover:bg-secondary/50",
  };
...
export function StatCard({ label, value, sub, tone = "default" }: { label: string; value: string | number; sub?: string; tone?: "default" | "ok" | "warn" | "danger" }) {
  const toneCls = {
