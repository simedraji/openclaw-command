...
function DesignPage() {
  const [prompt, setPrompt] = useState(
    "Retro-futurist illustration of a bearded dad, varsity jacket, 80s sunset, GIRL DAD ERA type, transparent PNG, 4500x5400",
  );
  return (
      ...
      <Panel>
        <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end">
          ...
          <OpenClawTaskButton
            ...
            message={`Prepare a production-ready POD design plan for this brief: ${prompt}. Use the configured image-generation tool if available. Return the generated asset path or a clear explanation of what is missing. Preserve transparent background, 4500x5400 px, 300 DPI, and do not publish externally.`}
          >
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 text-[12px]">
          <label className="flex items-center gap-2">
            <input type="checkbox" defaultChecked className="accent-primary" /> Background remover
          </label>
          ...
          <div className="ml-auto flex gap-2">
            ...
            <Btn size="sm">
              <Download className="h-3 w-3" /> Prompt JSON
            </Btn>
          </div>
        </div>
      </Panel>
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="panel overflow-hidden">
            <div className="relative aspect-square bg-gradient-to-br from-secondary via-background to-secondary/40">
              ...
              <div className="absolute left-2 top-2 rounded bg-background/70 px-1.5 py-0.5 font-mono text-[10px] backdrop-blur">
                seed_{88200 + i}
              </div>
            </div>
